 ...
 * @note Use esta funcao somente se ativou o modo de Bloco de endereçamento X-Y bit[2]=0b0
 */
void Panel_RA8889::Memory_ColorDepth_BlockMode(eColorDepthBPP colordepth)
{
...
void Panel_RA8889::ShowPage(uint8_t page)
{
#if defined(COLOR_DEPTH_8)
  Select_MainWindow_8bpp();
#elif defined(COLOR_DEPTH_16)
 ...
 *
 */
void Panel_RA8889::ShowPicturePgm(uint32_t size, const uint8_t *datap)
{   
  uint32_t i;
  SPI_CmdWrite(REG_MRWDP);                            //0x04, Memory Data Read/Write Port (MRWDP)
#if defined(COLOR_DEPTH_8)
 ...
 *
 */
void Panel_RA8889::DrawPicturePgm(uint16_t Wx, uint16_t Hy, uint16_t width, uint16_t height, const uint8_t *datap)
{
 ...
 *       24bpp: R8G8B8
 */
void Panel_RA8889::ShowPicture(eColorDepthBPP pictureBpp, uint32_t numpixels, const uint8_t *datap)
{   
  uint32_t i;
  SPI_CmdWrite(REG_MRWDP);                            //0x04, Memory Data Read/Write Port (MRWDP)
...
//Não testado
void Panel_RA8889::DrawBitmap(uint8_t *pixels, eColorDepthBPP pictureBpp, uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  ActiveWindow_XY(x, y);
  ActiveWindow_WidhtHeight(w, h);
  GotoPixel_XY(x, y);

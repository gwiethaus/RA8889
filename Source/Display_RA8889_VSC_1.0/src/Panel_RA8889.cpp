 ...
 */
bool Panel_RA8889::RA8889_ReadIDCode(void)
 { 
  uint8_t datareg = 0;
  uint8_t temp;
 ...
 *
 */
void Panel_RA8889::Mask_XnWAIT(bool mask)
{
  uint8_t temp;
...
void Panel_RA8889::ExtInterrupt_NoDebounce(void)
{
  ...
  SPI_CmdWrite(REG_ICR);                       //0x03, Input Control Register (ICR)
  temp = SPI_DataRead();                       
  //temp &= cSetb6;     //aqui deve ser cClrb6
  temp &= cClrb6;     //aqui deve ser cClrb6
  SPI_DataWrite(temp);
}
...
uint32_t Panel_RA8889::LayerStartAddr(uint8_t layer)
{
  ...
  return _width * _height * (static_cast<uint8_t>(_bpp) / 8) * layer;   //ex. 800x480 * (16 (16bpp)/8) * 1 = 768000 = 0xbb800
}
...
void Panel_RA8889::TriangleMode_Start(bool fill)
{
  ...
  fill ? temp |= cClrb5 : temp &= cClrb5; //Set bit 5, Com preenchimento do triangulo
...
void Panel_RA8889::CircleMode_Start(bool fill)
{
  ...
  fill ?  temp |= cSetb6 : temp &= cClrb6;     //Set bit 6 = Fill, Reset bit 6 = Non-Fill
...
{ 
  ...
  SPI_CmdWrite(REG_BTE_COLR);                     //0x92, Source 0/1 & Destination Color Depth (BTE_COLR)
  temp = SPI_DataRead();
  temp &= cClrb6;                                 //Reset bits 6
  temp |= cSetb5;                                 //Set bits 5
  SPI_DataWrite(temp);
} 
...
{ 
  ...
  SPI_CmdWrite(REG_BTE_COLR);                     //0x92, Source 0/1 & Destination Color Depth (BTE_COLR)
  temp = SPI_DataRead();
  temp |= cSetb6;                                 //Set bits 6
  temp &= cClrb5;                                 //Reset bits 5
  SPI_DataWrite(temp);
} 
...
{ 
  ...
  SPI_CmdWrite(REG_BTE_COLR);                     //0x92, Source 0/1 & Destination Color Depth (BTE_COLR)
  temp = SPI_DataRead();
  temp &= cClrb1;                                 //Reseta bits 1
  temp |= cSetb0;                                 //Set bits 0
  SPI_DataWrite(temp);
} 
...
{ 
  ...
  SPI_CmdWrite(REG_BTE_COLR);                     //0x92, Source 0/1 & Destination Color Depth (BTE_COLR)
  temp = SPI_DataRead();
  temp |= cSetb1;                                 //Set bits 1
  temp &= cClrb0;                                 //Reset bits 0
  SPI_DataWrite(temp);
} 
...
void Panel_RA8889::Select_SFI_SingleData_03h(void)
{
  ...
  SPI_CmdWrite(REG_DMA_CTRL);                  //0xb6, Serial flash DMA Controller REG (DMA_CTRL)
  temp = SPI_DataRead();                       
  //temp &= (cClrb7 | cClrb6);                   //Use [0xb7] bit [3-0] <<--esta errado
  temp &= ~(cSetb7 | cSetb6);                    //Reset bit 7 e 6, Use [0xb7] bit [3-0]
  SPI_DataWrite(temp);
...
void Panel_RA8889::DrawPicture(uint16_t Wx, uint16_t Hy, uint16_t width, uint16_t height, const uint8_t *datap)
{
  ...
  GotoPixel_XY(Wx, Hy);
  ShowPicture(width * height * (static_cast<uint8_t>(_bpp) / 8), *datap);
}

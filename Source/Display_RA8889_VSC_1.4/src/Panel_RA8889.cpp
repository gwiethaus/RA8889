...
//Chip Configuration Register (CCR) [01h]
  ...
  #endif
  #ifdef MCU8
    HostDataBus_Select_8bit();                   //MCU/MPU Host bus 16bit
  #endif
...
uint8_t Panel_RA8889::BPP(void) { return _bpp; }
...
void Panel_RA8889::SPI_DataWrite16(uint16_t data)
{
  SPISetCS(0);                                 //SS_RESET;
  SPIRwByte(RA8889_SPI_DATAWRITE);             //0x80, Indica Dados para escrever
  SPIRwByte(data & 0xff);                      //Envia um byte menos significativo de Dado para o SPI
  SPIRwByte((data >> 8) & 0xff);               //Envia um byte mais significativo de Dado para o SPI
  SPISetCS(1);                                 //SS_SET;
}
 ...
 * @endverbatim
 * 
 * @param data: dados de 3 bytes para display
 *
 * @note Note
 */
void Panel_RA8889::SPI_DataWrite24(uint32_t data)
{
  SPISetCS(0);                                 //SS_RESET;
  SPIRwByte(RA8889_SPI_DATAWRITE);             //0x80, Indica Dados para escrever 
  SPIRwByte(data & 0xff);                      //Envia byte 1 de Dado para o SPI
  SPIRwByte((data >> 8) & 0xff);               //Envia byte 2 de Dado para o SPI
  SPIRwByte((data >> 16) & 0xff);              //Envia byte 3 de Dado para o SPI
  SPISetCS(1);                                 //SS_SET;
}
...
/** OK
 * @brief Host MPU/MCU Read/Write Data Format
 *        MPU/MCU Read/Write data format when access memory data por
 *
 * @verbatim
...
void Panel_RA8889::MPU8_8bpp_MemoryWrite(
                                         ...
                                         uint16_t w ,                   // width
                                         uint16_t h ,                   // height
                                         const uint8_t *data_array      // 8bit data
                                        )
{
  ...
  for (i = 0; i < h; i++) {                    //Varredura da linha
    for (j = 0; j < w; j++) {                  //Varredura da coluna
      Wait_WriteFIFO_NotFull();                //Aguarde
      SPI_DataWrite(*data_array);              //Trasfere o byte apotnado no array de dados
      data_array++;                            //incrementa o potneiro apra a poxima posição
    }
  }
...
void Panel_RA8889::MPU16_16bpp_MemoryWrite(
                                           ...
                                           uint16_t w ,                   // width
                                           uint16_t h ,                   // height
                                           const uint16_t *data_array     // 8bit data
                                          )
{
  uint16_t i;                                  //posicao da linha
  uint16_t j;                                  //posicao da coluna
  ...
  for (i = 0; i < h; i++) {
    for (j = 0; j < w; j++) {
      Wait_WriteFIFO_NotFull();                //Aguarde
      SPI_DataWrite(*data_array);              //Trasfere o byte apontado no array de dados
      data_array++;                            //incrementa o potneiro apra a poxima posição
    }
    }
  Wait_WriteFIFO_Empty();
...
void Panel_RA8889::MPU16_24bpp_Mode1_MemoryWrite(uint16_t x,uint16_t y, uint16_t w , uint16_t h , const uint16_t *data_array)
{

}

//Escrita de Memoria de Display com MPU 16 bits / color depth 24bpp modo 2
void Panel_RA8889::MPU16_24bpp_Mode2_MemoryWrite(uint16_t x,uint16_t y, uint16_t w , uint16_t h , const uint16_t *data_array)
{
...
void Panel_RA8889::DMA_24bit_Block (
                                    uint8_t scs,         // scs: Select SPI : SCS0/SCS1
...
void Panel_RA8889::PIP(
                       bool On_Off,                   // 0 : disable PIP, 1 : enable PIP, 2 : To maintain the original state
 ...
 */
void Panel_RA8889::PWM0( 
                        bool on_off,            // true ON pwm, false OFF pwm
                           ...
                           )
{
  ...
  #ifdef COLOR_DEPTH_16
    SPI_DataWrite16(color);
  #endif
  #ifdef COLOR_DEPTH_24
    SPI_DataWrite24(color);
  #endif
  #if USE_XNWAIT
...
//Funcao em construção... baseado no sumotoy
/**
 ...
 * @param forecolor cor de frente do texto
 * @param backcolor cor de fundo do texto
 * 
 * @note None
 *
 */
void Panel_RA8889::Text(uint16_t x, uint16_t y, char *str, uint32_t forecolor, uint32_t backcolor)
{
  uint8_t mode = IsGraphicMode();  //Veja ese esta em modo grafico
  TextMode();                      //Modo texto 
#ifdef COLOR_DEPTH_16
  ForegroundColor16bpp(forecolor);
  BackgroundColor16bpp(backcolor);
#endif
#ifdef COLOR_DEPTH_24
  ForegroundColor24bpp(forecolor);
  BackgroundColor24bpp(backcolor);
#endif

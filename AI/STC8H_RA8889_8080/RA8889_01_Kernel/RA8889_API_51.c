 ...
 *					（3）NAND FLASH功能函数移植起来相对复杂，暂时屏蔽
 *					（4）删除调用自建ASCII字库的相关函数
  *					（5）修正部份函数，8位机计算整型变量可能会溢出，改为长整型
...
void MPU8_8bpp_Memory_Write(
    ...
    ,
    const unsigned char *data_array // 8bit data
)
{
    unsigned short i, j;
    Graphic_Mode();
...
void MPU16_16bpp_Memory_Write(
    ...
    unsigned short h // height
    ,
    const unsigned short *data_array // 16bit data
)
{
    unsigned short i, j;
    Graphic_Mode();
...
)
{
    ...
    /* Aribrial */
    Set_GTFont_Decoder(0x3a);
    LCD_CmdWrite(0x04);
    LCD_DataWrite(0x060C);
    Check_2D_Busy();
...
)
{
    ...
    if (ChromaKey == 1) { Font_Background_select_Original_Canvas(); }
...
void BTE_MCU_Write_MCU_8bit(
    ...
    unsigned short Y_H // Length of BTE Window
    ,
    const unsigned char *data_array // 8-bit data
)
{
    unsigned short i, j;
...
void BTE_MCU_Write_MCU_16bit(
    ...
    unsigned short Y_H // Length of BTE Window
    ,
    const unsigned short *data_array // 16-bit data
)
{
    unsigned short i, j;
...
)
{
    ...
    Set_Timer0_Count_Buffer(Count_Buffer);     // 砞﹚顶计
...
)
{
    ...
    Set_Timer1_Count_Buffer(Count_Buffer);     // 砞﹚顶计
...
//    nSS_Inactive();
//    delay_ms(1); // delay for buffer load

//    IDEC_Source_Start_Address(addr); // JPG癬cl|i
...
//    ,
//    unsigned int picture_Width, unsigned int picture_Height, unsigned long pic_buffer_Layer // pic_buffer_Layer : read pic buffer in sdram
//    ,
//    unsigned long Show_pic_Layer // Show_pic_Layer : write pic into sdram addr
//    ,
...
{
  BTE_S0_Color_24bpp();															//S0的色深选择
  BTE_S0_Memory_Start_Address(S0_Addr);
  BTE_S0_Image_Width(S0_W);
  BTE_S0_Window_Start_XY(XS0,YS0);									//S0与Des位置设定一样

  BTE_S1_Color_16bit_Alpha();												//选用16bpp，固定不变
  BTE_S1_Memory_Start_Address(S1_Addr);
  BTE_S1_Image_Width(S1_W); 
  BTE_S1_Window_Start_XY(XS1,YS1);
  
  BTE_Destination_Color_24bpp();										//Des的色深选择
  BTE_Destination_Memory_Start_Address(Des_Addr);
  BTE_Destination_Image_Width(Des_W);
  BTE_Destination_Window_Start_XY(XDes,YDes);

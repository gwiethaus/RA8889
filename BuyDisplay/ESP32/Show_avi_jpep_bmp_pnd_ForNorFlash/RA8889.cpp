...
void ER_TFTBasic::ER_TFTBasic::SPISetCs(int cs)
{
	if(cs)
...
void ER_TFTBasic::ER_TFTBasic::SPI_CmdWrite(int cmd)
{
...
void ER_TFTBasic::ER_TFTBasic::SPI_DataWrite(int data)
{
  ER_TFT.SPISetCs(0);    //SS_RESET;
...
void ER_TFTBasic::ER_TFTBasic::SPI_DataWrite_Pixel(int data)
{
  ER_TFT.SPISetCs(0);    //SS_RESET;
  ER_TFT.SPIRwByte(0x80);
  ER_TFT.SPIRwByte(data);
...
void ER_TFTBasic::ER_TFTBasic::Parallel_Init(void)
{
	#if Arduino_SPI
	ER_TFT.SPIInit();
	#endif
...
void ER_TFTBasic::ER_TFTBasic::LCD_CmdWrite(unsigned char cmd)
{
	#if Arduino_SPI
...
void ER_TFTBasic::ER_TFTBasic::LCD_DataWrite(unsigned char data)
{
	#if Arduino_SPI
...
void ER_TFTBasic::ER_TFTBasic::LCD_DataWrite_Pixel(unsigned int data)
{
	#if Arduino_SPI
...
void ER_TFTBasic::ER_TFTBasic::LCD_RegisterWrite(unsigned char Cmd,unsigned char Data)
{
...
//**[01h]**//
    ER_TFT.TFT_24bit();
  ER_TFT.Host_Bus_8bit(); //Host bus 16bit
      
//**[02h]**//
  ER_TFT.Data_Format_8b_16bpp();
...
//**[03h]**//
  ...
  ER_TFT.Select_Main_Window_16bpp();		//[10h]Set main window color depth
  ER_TFT.Memory_16bpp_Mode();			//[5Eh]Set active memory color depth

  ER_TFT.Select_PIP1_Window_16bpp();		//[11h] PIP 1 Window Color Depth
  ER_TFT.Select_PIP2_Window_16bpp();		//[11h] PIP 2 Window Color Depth

  ER_TFT.BTE_S0_Color_16bpp();			//[92h] Source_0 Color Depth
  ER_TFT.BTE_S1_Color_16bpp();			//[92h] Source_1 Color Depth
  ER_TFT.BTE_Destination_Color_16bpp();	//[92h] Destination Color Depth	 
		
  ER_TFT.IDEC_Destination_Color_16bpp();// 
...
void ER_TFTBasic::Check_IC_ready(void)
{
...
*/	
	...
	for(i=0;i<1000;i++)
	{
		if( (ER_TFT.LCD_StatusRead()&0x02)==0x00 ){break;}
	}
#endif	
}
...
#ifdef Check_While_Busy
	while( (ER_TFT.LCD_StatusRead()&0x04) == 0x00 );
#else
	// Please according to your usage to modify delay(check) time.
	for(i=0;i<1000;i++)
	{
		if( (ER_TFT.LCD_StatusRead()&0x04)==0x04 ){break;}
	}
#endif	
}
...
void ER_TFTBasic::Check_Mem_WR_FIFO_not_Full(void)
{
    ...
    */
	...
	for(i=0;i<1000;i++)
	{
		if( (ER_TFT.LCD_StatusRead()&0x80)==0 ){break;}
	}
#endif	
}
...
void ER_TFTBasic::Check_Mem_WR_FIFO_Empty(void)
{
    ...
    */
	...
	for(i=0;i<1000;i++)
	{
		if ((ER_TFT.LCD_StatusRead() & 0x40) == 0x40) { break; }
	}
#endif	
}
...
void ER_TFTBasic::Check_Mem_RD_FIFO_not_Full(void)
{
    ...
    */
	...
	for(i=0;i<1000;i++)
	{
		if ((ER_TFT.LCD_StatusRead() & 0x20) == 0x00) { break; }
	}
#endif	
}
...
void ER_TFTBasic::Check_Mem_RD_FIFO_Full(void)
{
    /*	[Status Register] bit5
    Host Memory Read FIFO full
    0: Memory Read FIFO is not full.
...
#ifdef Check_While_Busy
	while ((ER_TFT.LCD_StatusRead() & 0x20) == 0x00);
#else
	// Please according to your usage to modify delay(check) time. // 請依據您的使用情況修改執行時間
	for(i=0;i<1000;i++)
	{
		if ((ER_TFT.LCD_StatusRead() & 0x20) == 0x20) { break; }
	}
#endif	
}
...
void ER_TFTBasic::Check_Mem_RD_FIFO_not_Empty(void)
{
    ...
    */
	...
	for(i=0;i<1000;i++)
	{
		if ((ER_TFT.LCD_StatusRead() & 0x10) == 0x00) { break; }
	}
#endif	
}
...
void ER_TFTBasic::Check_2D_Busy(void)
{
    ...
    */
    ...
    for (i = 0; i < 1000; i++)
		{
        if ((ER_TFT.LCD_StatusRead() & 0x08) == 0x00) { break; }
    }
#endif
}
...
unsigned char Power_Saving_Status(void)
{
    ...
    */
    ...
    if ((ER_TFT.LCD_StatusRead() & 0x02) == 0x02)
        temp = 1;
    else
        temp = 0;
...
void ER_TFTBasic::Check_Power_is_Saving(void)
{
    /*	[Status Register] bit1
    Operation mode status
    0: Normal operation state
...
#ifdef Check_While_Busy
	while ((ER_TFT.LCD_StatusRead() & 0x02) == 0x00);
#else
	// Please according to your usage to modify delay(check) time. // 請依據您的使用情況修改執行時間
	for (i = 0; i < 1000; i++)
	{
		if ((ER_TFT.LCD_StatusRead() & 0x02) == 0x02) { break; }
	}
#endif		
}


void ER_TFTBasic::Check_NO_Interrupt(void)
{
...
#ifdef Check_While_Busy
	while (ER_TFT.LCD_StatusRead() & 0x01);
#else
	// Please according to your usage to modify delay(check) time. // 請依據您的使用情況修改執行時間
	for (i = 0; i < 1000; i++)
	{
		if ((ER_TFT.LCD_StatusRead() & 0x01) == 0x00) { break; }
	}
#endif	
}


void ER_TFTBasic::Check_Interrupt_Occur(void)
{
    /*	[Status Register] bit0
    Interrupt pin state
    0: without interrupt active
...
#ifdef Check_While_Busy
	while ((ER_TFT.LCD_StatusRead() & 0x01) == 0x00);
#else
    // Please according to your usage to modify delay(check) time. // 請依據您的使用情況修改執行時間
	for (i = 0; i < 1000; i++)
	{
		if ((ER_TFT.LCD_StatusRead() & 0x01) == 0x01) { break; }
	}
#endif	
}
...
//void ER_TFTBasic::Set_LCD_Panel(void)
//{
//	//**[10h]**//
...
void ER_TFTBasic::Enable_PLL(void)
{
...
void ER_TFTBasic::Sent_XnWAIT_Status_When_CS_Low(void)
{ /*
...
void ER_TFTBasic::Sent_XnWAIT_Status_At_Any_Time(void)
{ /*
     Mask XnWAIT on XnCS deassert
     0 : No mask
         XnWAIT keep assert if internal state keep busy and cannot
...
void ER_TFTBasic::Key_Scan_Enable(void)
{
    /*  0: Disable.
        1: Enable.*/
    unsigned char temp;
...
void ER_TFTBasic::Key_Scan_Disable(void)
{
    /*  0: Disable.
        1: Enable.*/
    unsigned char temp;
...
void ER_TFTBasic::TFT_24bit(void) //
{
...
void ER_TFTBasic::Without_TFT(void)
{
    /*  TFT Panel I/F Output pin Setting
    00b: 24-bits TFT output.
    01b: 18-bits TFT output.
...
void ER_TFTBasic::RA8889_I2CM_Enable(void)
{
...
void ER_TFTBasic::RA8889_I2CM_Disable(void)
{
    /*  I2C master Interface Enable/Disable
            0: Disable (GPIO function)
            1: Enable (I2C master function)*/
...
void ER_TFTBasic::Disable_SFlash_SPI(void)
{
    /*	Serial Flash or SPI Interface Enable/Disable
            0: Disable (GPIO function)
            1: Enable (SPI master function)
...
void ER_TFTBasic::Host_Bus_8bit(void)
{
...
void ER_TFTBasic::Data_Format_8b_8bpp(void)
{
...
void ER_TFTBasic::Data_Format_8b_16bpp(void)
{
    /* MPU read/write data format when access memory data port.
    0xb: Direct write for
        all 8 bits MPU I/F,
...
void ER_TFTBasic::Data_Format_8b_24bpp(void)
{
    /* MPU read/write data format when access memory data port.
    0xb: Direct write for
        all 8 bits MPU I/F,
...
void ER_TFTBasic::Data_Format_16b_8bpp(void)
{
    /* MPU read/write data format when access memory data port.
...
void ER_TFTBasic::Data_Format_16b_16bpp(void)
{
    /* MPU read/write data format when access memory data port.
    0xb: Direct write for
        all 8 bits MPU I/F,
...
void ER_TFTBasic::Data_Format_16b_24bpp_mode1(void)
{
    /* MPU read/write data format when access memory data port.
    0xb: Direct write for
        all 8 bits MPU I/F,
...
void ER_TFTBasic::Data_Format_16b_24bpp_mode2(void)
{
    /* MPU read/write data format when access memory data port.
...
void ER_TFTBasic::MemRead_Right_Left_Top_Down(void)
{
    /* Host Read Memory Direction (Only for Graphic Mode)
...
void ER_TFTBasic::MemRead_Top_Down_Left_Right(void)
{
    /* Host Read Memory Direction (Only for Graphic Mode)
...
void ER_TFTBasic::MemRead_Down_Top_Left_Right(void)
{
    /* Host Read Memory Direction (Only for Graphic Mode)
...
void ER_TFTBasic::MemWrite_Right_Left_Top_Down(void)
{
    /* Host Write Memory Direction (Only for Graphic Mode)
    01b: Right .. Left then Top .. Bottom.
    Ignored if canvas in linear addressing mode.		*/
...
void ER_TFTBasic::MemWrite_Top_Down_Left_Right(void)
{
    /* Host Write Memory Direction (Only for Graphic Mode)
    10b: Top .. Bottom then Left .. Right.
    Ignored if canvas in linear addressing mode.		*/
...
void ER_TFTBasic::MemWrite_Down_Top_Left_Right(void)
{
    /* Host Write Memory Direction (Only for Graphic Mode)
    11b: Bottom .. Top then Left .. Right.
    Ignored if canvas in linear addressing mode.		*/
...
void ER_TFTBasic::Interrupt_Active_Low(void)
{
...
void ER_TFTBasic::Interrupt_Active_High(void)
{
    /*  Output to MPU Interrupt active level
        0 : active low.
        1 : active high.						*/
...
void ER_TFTBasic::ExtInterrupt_Debounce(void)
{
...
void ER_TFTBasic::ExtInterrupt_Nodebounce(void)
{
    /*  External interrupt input (XPS[0] pin) de-bounce
        0 : without de-bounce
        1 : enable de-bounce (1024 OSC clock)			*/
...
void ER_TFTBasic::ExtInterrupt_Input_Low_Level_Trigger(void)
{
...
void ER_TFTBasic::ExtInterrupt_Input_High_Level_Trigger(void)
{
    /* External interrupt input (XPS[0] pin) trigger type
...
void ER_TFTBasic::ExtInterrupt_Input_Falling_Edge_Trigger(void)
{
    /* External interrupt input (XPS[0] pin) trigger type
...
void ER_TFTBasic::ExtInterrupt_Input_Rising_Edge_Trigger(void)
{
    /* External interrupt input (XPS[0] pin) trigger type
...
void ER_TFTBasic::LVDS_Format1(void) // RA8877 only
{
...
void ER_TFTBasic::LVDS_Format2(void) // RA8877 only
{
    /* FPD-Link Data Format / LVDS Data Format
    0 : Format 1 (VESA format)
    1 : Format 2 (JEIDA format) 				*/
...
void ER_TFTBasic::Memory_Select_Gamma_Table(void)
{
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x03);
    temp = ER_TFT.LCD_DataRead();
...
void ER_TFTBasic::Memory_Select_Graphic_Cursor_RAM(void)
{
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x03);
    temp = ER_TFT.LCD_DataRead();
...
void ER_TFTBasic::Memory_Select_Color_Palette_RAM(void)
{
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x03);
    temp = ER_TFT.LCD_DataRead();
...
void ER_TFTBasic::Enable_Resume_Interrupt(void)
{
    /*
    ...
    */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x0B);
    temp = ER_TFT.LCD_DataRead();
    temp |= cSetb7;
    ER_TFT.LCD_DataWrite(temp);
}
void ER_TFTBasic::Disable_Resume_Interrupt(void)
{
    /*
    Wakeup/resume Interrupt Enable
    0: Disable.
...
void ER_TFTBasic::Enable_ExtInterrupt_Input(void)
{
    /*
...
void ER_TFTBasic::Disable_ExtInterrupt_Input(void)
{
    /*
    External Interrupt (PS[0] pin) Enable
    0: Disable.
...
void ER_TFTBasic::Enable_I2CM_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Disable_I2CM_Interrupt(void)
{
    /*
    I2C Master Interrupt Enable
    0: Disable.
...
void ER_TFTBasic::Enable_Vsync_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Disable_Vsync_Interrupt(void)
{
    /*
    Vsync time base interrupt Enable Bit
    0: Disable Interrupt.
...
void ER_TFTBasic::Enable_KeyScan_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Disable_KeyScan_Interrupt(void)
{
    /*
    Key Scan Interrupt Enable Bit
    0: Disable Key scan interrupt.
...
void ER_TFTBasic::Enable_DMA_Draw_BTE_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Disable_DMA_Draw_BTE_Interrupt(void)
{
    /*
    Serial flash DMA Complete | Draw task finished | BTE Process
    Complete etc. Interrupt Enable
...
void ER_TFTBasic::Enable_PWM1_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Disable_PWM1_Interrupt(void)
{
    /*
    PWM timer 1 Interrupt Enable Bit
    0: Disable Interrupt.
...
void ER_TFTBasic::Enable_PWM0_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Disable_PWM0_Interrupt(void)
{
    /*
    PWM timer 0 Interrupt Enable Bit
    0: Disable Interrupt.
...
unsigned char Read_Interrupt_status(void)
{
    ...
    */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x0C);
    temp = ER_TFT.LCD_DataRead();
    return temp;
}

void ER_TFTBasic::Check_Vsync_finished(void)
{
    /*[Bit4]
...
void ER_TFTBasic::Clear_Resume_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::Clear_ExtInterrupt_Input_Flag(void)
{
    /*
...
void ER_TFTBasic::Clear_I2CM_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::Clear_Vsync_Interrupt_Flag(void)
{
    /*
    Vsync Time base interrupt flag
...
void ER_TFTBasic::Clear_KeyScan_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::Clear_DMA_Draw_BTE_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::Clear_PWM1_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::Clear_PWM0_Interrupt_Flag(void)
{
    /*
...
//[0Dh]=========================================================================
void ER_TFTBasic::XnINTR_Mask_Resume_Interrupt_Flag(void)
{
    /*
    ...
    */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x0D);
    temp = ER_TFT.LCD_DataRead();
    temp |= cSetb7;
    ER_TFT.LCD_DataWrite(temp);
}
void ER_TFTBasic::XnINTR_Mask_ExtInterrupt_Input_Flag(void)
{
    /*
...
void ER_TFTBasic::XnINTR_Mask_I2CM_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::XnINTR_Mask_Vsync_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::XnINTR_Mask_KeyScan_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::XnINTR_Mask_DMA_Draw_BTE_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::XnINTR_Mask_PWM1_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::XnINTR_Mask_PWM0_Interrupt_Flag(void)
{
    /*
...
void ER_TFTBasic::XnINTR_Unmask_Resume_Interrupt_Flag(void)
{
    /*
    Mask Resume Interrupt Flag
    0: Enable.
...
void ER_TFTBasic::XnINTR_Unmask_ExtInterrupt_Input_Flag(void)
{
    /*
    Mask External Interrupt (PS[0] pin) Flag
    0: Enable.
...
void ER_TFTBasic::XnINTR_Unmask_I2CM_Interrupt_Flag(void)
{
    /*
    Mask I2C Master Interrupt Flag
    0: Enable.
...
void ER_TFTBasic::XnINTR_Unmask_Vsync_Interrupt_Flag(void)
{
    /*
    Mask Vsync time base interrupt Flag
    0: Enable.
...
void ER_TFTBasic::XnINTR_Unmask_KeyScan_Interrupt_Flag(void)
{
    /*
    Mask Key Scan Interrupt Flag
    0: Enable.
...
void ER_TFTBasic::XnINTR_Unmask_DMA_Draw_BTE_Interrupt_Flag(void)
{
    /*
    Mask Serial flash DMA Complete | Draw task finished | BTE
    Process Complete etc. Interrupt Flag
...
void ER_TFTBasic::XnINTR_Unmask_PWM1_Interrupt_Flag(void)
{
    /*
    Mask PWM timer 1 Interrupt Flag
    0: Enable.
...
void ER_TFTBasic::XnINTR_Unmask_PWM0_Interrupt_Flag(void)
{
    /*
    Mask PWM timer 0 Interrupt Flag
    0: Enable.
...
//[0Eh]=========================================================================
void ER_TFTBasic::Enable_GPIOF_PullUp(void)
{
    /*
    ...
    */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x0E);
    temp = ER_TFT.LCD_DataRead();
    temp |= cSetb5;
    ER_TFT.LCD_DataWrite(temp);
}
void ER_TFTBasic::Enable_GPIOE_PullUp(void)
{
    /*
...
void ER_TFTBasic::Enable_GPIOD_PullUp(void)
{
    /*
...
void ER_TFTBasic::Enable_GPIOC_PullUp(void)
{
    /*
...
void ER_TFTBasic::Enable_XDB15_8_PullUp(void)
{
    /*
...
void ER_TFTBasic::Enable_XDB7_0_PullUp(void)
{
    /*
...
void ER_TFTBasic::Disable_GPIOF_PullUp(void)
{
    /*
    GPIO_F[7:0] Pull-Up Enable (XPDAT[23:19, 15:13])
    0: Pull-Up Disable
...
void ER_TFTBasic::Disable_GPIOE_PullUp(void)
{
    /*
    GPIO_E[7:0] Pull-Up Enable (XPDAT[12:10, 7:3])
    0: Pull-Up Disable
...
void ER_TFTBasic::Disable_GPIOD_PullUp(void)
{
    /*
    GPIO_D[7:0] Pull-Up Enable (XPDAT[18, 2, 17, 16, 9, 8, 1,0])
    0: Pull-Up Disable
...
void ER_TFTBasic::Disable_GPIOC_PullUp(void)
{
    /*
    GPIO_C[6:0] Pull-Up Enable (XSDA, XSCL, XnSFCS1,
    XnSFCS0, XMISO, XMOSI , XSCK)
...
void ER_TFTBasic::Disable_XDB15_8_PullUp(void)
{
    /*
    XDB[15:8] Pull-Up Enable
    0: Pull-Up Disable
...
void ER_TFTBasic::Disable_XDB7_0_PullUp(void)
{
    /*
    XDB[7:0] Pull-Up Enable
    0: Pull-Up Disable
...
//[0Fh]=========================================================================
void ER_TFTBasic::XPDAT18_Set_GPIO_D7(void)
{
    /*
    ...
    */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x0F);
    temp = ER_TFT.LCD_DataRead();
    temp &= cClrb7;
    ER_TFT.LCD_DataWrite(temp);
}
void ER_TFTBasic::XPDAT18_Set_KOUT4(void)
{
    /*
    XPDAT[18] – not scan function select
    0: GPIO-D7
...
void ER_TFTBasic::XPDAT17_Set_GPIO_D5(void)
{
    /*
...
void ER_TFTBasic::XPDAT17_Set_KOUT2(void)
{
    /*
    XPDAT[17] – not scan function select
    0: GPIO-D5
...
void ER_TFTBasic::XPDAT16_Set_GPIO_D4(void)
{
    /*
...
void ER_TFTBasic::XPDAT16_Set_KOUT1(void)
{
    /*
    XPDAT[16] – not scan function select
    0: GPIO-D4
...
void ER_TFTBasic::XPDAT9_Set_GPIO_D3(void)
{
    /*
...
void ER_TFTBasic::XPDAT9_Set_KOUT3(void)
{
    /*
    XPDAT[9] – not scan function select
    0: GPIO-D3
...
void ER_TFTBasic::XPDAT8_Set_GPIO_D2(void)
{
    /*
...
void ER_TFTBasic::XPDAT8_Set_KIN3(void)
{
    /*
    XPDAT[8] – not scan function select
    0: GPIO-D2
...
void ER_TFTBasic::XPDAT2_Set_GPIO_D6(void)
{
    /*
...
void ER_TFTBasic::XPDAT2_Set_KIN4(void)
{
    /*
    XPDAT[2] – not scan function select
    0: GPIO-D6
...
void ER_TFTBasic::XPDAT1_Set_GPIO_D1(void)
{
    /*
...
void ER_TFTBasic::XPDAT1_Set_KIN2(void)
{
    /*
    XPDAT[1] – not scan function select
    0: GPIO-D1
...
void ER_TFTBasic::XPDAT0_Set_GPIO_D0(void)
{
    /*
...
void ER_TFTBasic::XPDAT0_Set_KIN1(void)
{
    /*
    XPDAT[0] – not scan function select
    0: GPIO-D0
...
//[10h]=========================================================================
void ER_TFTBasic::Enable_PIP1(void)
{
    /*
...
void ER_TFTBasic::Disable_PIP1(void)
{
    /*
    PIP 1 window Enable/Disable
    0 : PIP 1 window disable.
...
void ER_TFTBasic::Enable_PIP2(void)
{
    /*
...
void ER_TFTBasic::Disable_PIP2(void)
{
    /*
    PIP 2 window Enable/Disable
    0 : PIP 2 window disable.
...
void ER_TFTBasic::Select_PIP1_Parameter(void)
{
    /*
...
void ER_TFTBasic::Select_PIP2_Parameter(void)
{
    /*
    0: To configure PIP 1’s parameters.
    1: To configure PIP 2’s parameters..
...
void ER_TFTBasic::Select_Main_Window_8bpp(void)
{
    /*
...
void ER_TFTBasic::Select_Main_Window_24bpp(void)
{
    /*
    Main Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
...
void ER_TFTBasic::Select_LCD_Sync_Mode(void)
{
    /*
...
void ER_TFTBasic::Select_LCD_DE_Mode(void)
{
    /*
    To Control panel's synchronous signals
    0: Sync Mode: Enable XVSYNC, XHSYNC, XDE
...
//[11h]=========================================================================
void ER_TFTBasic::Select_PIP1_Window_8bpp(void)
{
    /*
    ...
    */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0x11);
    temp = ER_TFT.LCD_DataRead();
    temp &= cClrb3;
    temp &= cClrb2;
    ER_TFT.LCD_DataWrite(temp);
}
void ER_TFTBasic::Select_PIP1_Window_16bpp(void)
{
    /*
    PIP 1 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
...
void ER_TFTBasic::Select_PIP1_Window_24bpp(void)
{
    /*
    PIP 1 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
...
void ER_TFTBasic::Select_PIP2_Window_8bpp(void)
{
    /*
...
void ER_TFTBasic::Select_PIP2_Window_16bpp(void)
{
    /*
    PIP 2 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
...
void ER_TFTBasic::Select_PIP2_Window_24bpp(void)
{
    /*
    PIP 2 Window Color Depth Setting
    00b: 8-bpp generic TFT, i.e. 256 colors.
...
void ER_TFTBasic::Display_OFF(void)
{
    /*
    Display ON/OFF
    0b: Display Off.
...
void ER_TFTBasic::Color_Bar_ON(void)
{
    /*
...
void ER_TFTBasic::Color_Bar_OFF(void)
{
    /*
    Display Test Color Bar
    0b: Disable.
...
void ER_TFTBasic::HSCAN_R_to_L(void)
{
    /*
    Horizontal Scan direction
    0 : From Left to Right
...
void ER_TFTBasic::VSCAN_B_to_T(void)
{
    /*
    Vertical Scan direction
    0 : From Top to Bottom
...
void ER_TFTBasic::PDATA_Set_RBG(void)
{
    /*parallel PDATA[23:0] Output Sequence
    000b : RGB
    001b : RBG
...
void ER_TFTBasic::PDATA_Set_GRB(void)
{
    /*parallel PDATA[23:0] Output Sequence
    000b : RGB
    001b : RBG
...
void ER_TFTBasic::PDATA_Set_GBR(void)
{
    /*parallel PDATA[23:0] Output Sequence
    000b : RGB
    001b : RBG
...
void ER_TFTBasic::PDATA_Set_BRG(void)
{
    /*parallel PDATA[23:0] Output Sequence
    000b : RGB
    001b : RBG
...
void ER_TFTBasic::PDATA_Set_BGR(void)
{
    /*parallel PDATA[23:0] Output Sequence
    000b : RGB
    001b : RBG
...
void ER_TFTBasic::PDATA_Set_Gray(void)
{
    /*parallel PDATA[23:0] Output Sequence
    000b : RGB
    001b : RBG
...
void ER_TFTBasic::PDATA_IDLE_STATE(void)
{
    /*
...
void ER_TFTBasic::Idle_DE_Low(void)
{
    /*
...
void ER_TFTBasic::Idle_DE_High(void)
{
    /*
    DE IDLE STATE(When STANDBY or DISPLAY OFF )
    0 : Pin “DE” output is low.
...
void ER_TFTBasic::Idle_PCLK_Low(void)
{
    /*
...
void ER_TFTBasic::Idle_PCLK_High(void)
{
    /*
    PCLK IDLE STATE(When STANDBY or DISPLAY OFF )
    0 : Pin “PCLK” output is low.
...
void ER_TFTBasic::Idle_PDAT_Low(void)
{
    /*
...
void ER_TFTBasic::Idle_PDAT_High(void)
{
    /*
    PDAT IDLE STATE(When STANDBY or DISPLAY OFF )
    0 : Pins “PDAT[23:0]” output is low.
...
void ER_TFTBasic::Idle_HSYNC_Low(void)
{
    /*
...
void ER_TFTBasic::Idle_HSYNC_High(void)
{
    /*
    HSYNC IDLE STATE(When STANDBY or DISPLAY OFF )
    0 : Pin “HSYNC” output is low.
...
void ER_TFTBasic::Idle_VSYNC_Low(void)
{
    /*
...
void ER_TFTBasic::Idle_VSYNC_High(void)
{
    /*
    VSYNC IDLE STATE(When STANDBY or DISPLAY OFF )
    0 : Pin “VSYNC” output is low.
...
void ER_TFTBasic::Main_Image_Start_Address(unsigned long Addr)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x20);
    ER_TFT.LCD_DataWrite(Addr);
    ER_TFT.LCD_CmdWrite(0x21);
    ER_TFT.LCD_DataWrite(Addr >> 8);
    ER_TFT.LCD_CmdWrite(0x22);
    ER_TFT.LCD_DataWrite(Addr >> 16);
    ER_TFT.LCD_CmdWrite(0x23);
...
void ER_TFTBasic::Main_Image_Width(unsigned short WX)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x24);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x25);
...
void ER_TFTBasic::Main_Window_Start_XY(unsigned short WX, unsigned short HY)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x26);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x27);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x28);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x29);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
//[2Ah][2Bh][2Ch][2Dh]=========================================================================
void ER_TFTBasic::PIP_Display_Start_XY(unsigned short WX, unsigned short HY)
{
    /*
    ...
    Function bit will be configured for relative PIP window.
    */
    ER_TFT.LCD_CmdWrite(0x2A);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x2B);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x2C);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x2D);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
//[2Eh][2Fh][30h][31h]=========================================================================
void ER_TFTBasic::PIP_Image_Start_Address(unsigned long Addr)
{
    /*
    ...
    */
    ER_TFT.LCD_CmdWrite(0x2E);
    ER_TFT.LCD_DataWrite(Addr);
    ER_TFT.LCD_CmdWrite(0x2F);
    ER_TFT.LCD_DataWrite(Addr >> 8);
    ER_TFT.LCD_CmdWrite(0x30);
    ER_TFT.LCD_DataWrite(Addr >> 16);
    ER_TFT.LCD_CmdWrite(0x31);
    ER_TFT.LCD_DataWrite(Addr >> 24);
}
//[32h][33h]=========================================================================
void ER_TFTBasic::PIP_Image_Width(unsigned short WX)
{
    /*
    ...
    Function bit will be configured for relative PIP window.
    */
    ER_TFT.LCD_CmdWrite(0x32);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x33);
    ER_TFT.LCD_DataWrite(WX >> 8);
}
//[34h][35h][36h][37h]=========================================================================
void ER_TFTBasic::PIP_Window_Image_Start_XY(unsigned short WX, unsigned short HY)
{
    ...
    Function bit will be configured for relative PIP window.
    */
    ER_TFT.LCD_CmdWrite(0x34);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x35);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x36);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x37);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
//[38h][39h][3Ah][3Bh]=========================================================================
void ER_TFTBasic::PIP_Window_Width_Height(unsigned short WX, unsigned short HY)
{
    /*
    ...
    Function bit will be configured for relative PIP window.
    */
    ER_TFT.LCD_CmdWrite(0x38);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x39);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x3A);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x3B);
    ER_TFT.LCD_DataWrite(HY >> 8);
}

//[3Ch]=========================================================================
void ER_TFTBasic::Enable_Gamma_Correction(void)
{
    /*
    ...
    */
    ...
    ER_TFT.LCD_CmdWrite(0x3C);
    temp = ER_TFT.LCD_DataRead();
    temp |= cSetb7;
    ER_TFT.LCD_DataWrite(temp);
}
void ER_TFTBasic::Disable_Gamma_Correction(void)
{
    /*
    Gamma correction Enable
    0: Disable
...
void ER_TFTBasic::Gamma_Table_for_Blue(void)
{
    /*
...
void ER_TFTBasic::Gamma_Table_for_Green(void)
{
    /*
    Gamma table select for MPU write gamma data
    00b: Gamma table for Blue
...
void ER_TFTBasic::Gamma_Table_for_Red(void)
{
    /*
    Gamma table select for MPU write gamma data
    00b: Gamma table for Blue
...
void ER_TFTBasic::Enable_Graphic_Cursor(void)
{
    /*
...
void ER_TFTBasic::Disable_Graphic_Cursor(void)
{
    /*
    Graphic Cursor Enable
    0 : Graphic Cursor disable.
...
//
void ER_TFTBasic::Select_Graphic_Cursor_1(void)
{
    /*
...
void ER_TFTBasic::Select_Graphic_Cursor_2(void)
{
    /*
    Graphic Cursor Selection Bit
    Select one from four graphic cursor types. (00b to 11b)
...
void ER_TFTBasic::Select_Graphic_Cursor_3(void)
{
    /*
    Graphic Cursor Selection Bit
    Select one from four graphic cursor types. (00b to 11b)
...
void ER_TFTBasic::Select_Graphic_Cursor_4(void)
{
    /*
    Graphic Cursor Selection Bit
    Select one from four graphic cursor types. (00b to 11b)
...
//
void ER_TFTBasic::Enable_Text_Cursor(void)
{
    /*
    Text Cursor Enable
    0 : Disable.
...
void ER_TFTBasic::Disable_Text_Cursor(void)
{
    /*
    Text Cursor Enable
    0 : Disable.
...
//
void ER_TFTBasic::Enable_Text_Cursor_Blinking(void)
{
    /*
...
void ER_TFTBasic::Disable_Text_Cursor_Blinking(void)
{
    /*
    Text Cursor Blinking Enable
    0 : Disable.
...
//[3Dh]=========================================================================
void ER_TFTBasic::Blinking_Time_Frames(unsigned char temp)
{
    /*
    ...
    FFh : 256 frames time.
    */
    ER_TFT.LCD_CmdWrite(0x3D);
    ER_TFT.LCD_DataWrite(temp);
}
//[3Eh]=========================================================================
void ER_TFTBasic::Text_Cursor_H_V(unsigned short WX, unsigned short HY)
{
    /*
    [3Eh]
    ...
    same times as the font enlargement.
    */
    ER_TFT.LCD_CmdWrite(0x3E);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x3F);
    ER_TFT.LCD_DataWrite(HY);
}
//[40h][41h][42h][43h]=========================================================================
void ER_TFTBasic::Graphic_Cursor_XY(unsigned short WX, unsigned short HY)
{
    /*
    ...
    Reference main Window coordinate.
    */
    ER_TFT.LCD_CmdWrite(0x40);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x41);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x42);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x43);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
//[44h]=========================================================================
void ER_TFTBasic::Set_Graphic_Cursor_Color_1(unsigned char temp)
{
    /*
...
//[45h]=========================================================================
void ER_TFTBasic::Set_Graphic_Cursor_Color_2(unsigned char temp)
{
    /*
...
void ER_TFTBasic::Canvas_Image_Start_address(unsigned long Addr)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x50);
    ER_TFT.LCD_DataWrite(Addr);
    ER_TFT.LCD_CmdWrite(0x51);
    ER_TFT.LCD_DataWrite(Addr >> 8);
    ER_TFT.LCD_CmdWrite(0x52);
    ER_TFT.LCD_DataWrite(Addr >> 16);
    ER_TFT.LCD_CmdWrite(0x53);
    ER_TFT.LCD_DataWrite(Addr >> 24);
}
//[54h][55h]=========================================================================
void ER_TFTBasic::Canvas_image_width(unsigned short WX)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x54);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x55);
    ER_TFT.LCD_DataWrite(WX >> 8);
}
//[56h][57h][58h][59h]=========================================================================
void ER_TFTBasic::Active_Window_XY(unsigned short WX, unsigned short HY)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x56);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x57);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x58);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x59);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
//[5Ah][5Bh][5Ch][5Dh]=========================================================================
void ER_TFTBasic::Active_Window_WH(unsigned short WX, unsigned short HY)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x5A);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x5B);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x5C);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x5D);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
//[5Eh]=========================================================================
...
void ER_TFTBasic::Memory_8bpp_Mode(void)
{
    /*
...
void ER_TFTBasic::Memory_24bpp_Mode(void)
{
    /*
    Canvas image’s color depth & memory R/W data width
    In Block Mode:
...
void ER_TFTBasic::Goto_Pixel_XY(unsigned short WX, unsigned short HY)
{
    ...
    */
    ER_TFT.LCD_CmdWrite(0x5F);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x60);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x61);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x62);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
void ER_TFTBasic::Goto_Linear_Addr(unsigned long Addr)
{
    /*
    Set Graphic Read/Write position
...
void ER_TFTBasic::Goto_Text_XY(unsigned short WX, unsigned short HY)
{
    ...
    Unit: Pixel
    */
    ER_TFT.LCD_CmdWrite(0x63);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0x64);
    ER_TFT.LCD_DataWrite(WX >> 8);
    ER_TFT.LCD_CmdWrite(0x65);
    ER_TFT.LCD_DataWrite(HY);
    ER_TFT.LCD_CmdWrite(0x66);
    ER_TFT.LCD_DataWrite(HY >> 8);
}
...
1 : Draw Triangle
*/
void ER_TFTBasic::Draw_Enable_AA(void)
{
    unsigned char temp;
...
void ER_TFTBasic::Draw_Disable_AA(void)
{
    unsigned char temp;
...
//[92h]=========================================================================
void ER_TFTBasic::BTE_S1_Color_32bit_Alpha(void)
{
    /*
    S1 Color Depth
    000 : 256 Color
...
void ER_TFTBasic::BTE_Window_Size(unsigned short WX, unsigned short HY)
...
void ER_TFTBasic::Select_SFI_Quad_Mode_Dummy_8T_6Bh(void) //
{
    /*[B6h]bit7-6=
    00b: [B7h] B3-0
...
void ER_TFTBasic::Select_SFI_Quad_Mode_Dummy_4T_EBh(void) //
{
    /*[B6h]bit7-6=
    00b: [B7h] B3-0
    01b: 4x read command code – 6Bh.
...
void ER_TFTBasic::Select_standard_SPI_Mode0_or_Mode3(void)
{
    /*[bit4]
...
void ER_TFTBasic::Select_RA8875_SPI_Mode0_and_Mode3(void)
{
    /*[bit4]
    RA8875 compatible mode
    0: standard SPI mode 0 or mode 3 timing
...
void ER_TFTBasic::Select_SFI_Single_Mode_Dummy_0T_03h(void)
{
    /*
    (1) [B6h]bit7-6=0x00
    (2) [B7h]bit3-0=
...
void ER_TFTBasic::Select_SFI_Single_Mode_Dummy_8T_0Bh(void)
{
    /*
    (1) [B6h]bit7-6=0x00
    (2) [B7h]bit3-0=
...
void ER_TFTBasic::Select_SFI_Single_Mode_Dummy_16T_1Bh(void)
{
    /*
    (1) [B6h]bit7-6=0x00
    (2) [B7h]bit3-0=
...
void ER_TFTBasic::Select_SFI_Dual_Mode_Dummy_8T_3Bh(void)
{
    /*
    (1) [B6h]bit7-6=0x00
    (2) [B7h]bit3-0=
...
void ER_TFTBasic::Select_SFI_Dual_Mode_Dummy_4T_BBh(void)
{
    /*
    (1) [B6h]bit7-6=0x00
    (2) [B7h]bit3-0=
...
unsigned char ER_TFTBasic::SPI_Master_FIFO_Data_Put(unsigned char Data)
{
    ...
    ER_TFT.LCD_DataWrite(Data);
    do {
        temp = ER_TFT.Tx_FIFO_Empty_Flag();
    } while (temp == 0);
    //	while(Tx_FIFO_Empty_Flag()==0);	//空了再執行下一筆
    temp = ER_TFT.SPI_Master_FIFO_Data_Get();
...
unsigned char ER_TFTBasic::SPI_Master_FIFO_Data_Get(void)
{
    ...
    while (ER_TFT.Rx_FIFO_Empty_Flag() == 1)
        ; // 不是空的往下執行
    ER_TFT.LCD_CmdWrite(0xB8);
    temp = ER_TFT.LCD_DataRead();
...
void ER_TFTBasic::Enable_SPI_Master_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Disable_SPI_Master_Interrupt(void)
{
    /*
    SPI Master Interrupt enable
    0: Disable interrupt.
...
void ER_TFTBasic::Mask_FIFO_overflow_error_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Unmask_FIFO_overflow_error_Interrupt(void)
{
    /*
    Mask interrupt for FIFO overflow error [OVFIRQEN]
    0: unmask
...
void ER_TFTBasic::Mask_EMTIRQEN_Interrupt(void)
{
    /*
...
void ER_TFTBasic::Unmask_EMTIRQEN_Interrupt(void)
{
    /*
    Mask interrupt for while Tx FIFO empty & SPI engine/FSM idle [EMTIRQEN]
    0: unmask
...
//[C6h][C7h][C8h][C9h]=========================================================================
void ER_TFTBasic::SFI_DMA_Transfer_Number(unsigned long Num)
{
    /*
    When REG DMACR bit 1 = 0 (Linear Mode)
    ...
    */
    ...
    ER_TFT.LCD_DataWrite(Num);
    ER_TFT.LCD_CmdWrite(0xC7);
    ER_TFT.LCD_DataWrite(Num >> 8);
    ER_TFT.LCD_CmdWrite(0xC8);
    ER_TFT.LCD_DataWrite(Num >> 16);
    ER_TFT.LCD_CmdWrite(0xC9);
    ER_TFT.LCD_DataWrite(Num >> 24);
}
...
void ER_TFTBasic::Internal_CGROM_Select_ISOIEC8859_5(void)
{
    /*
    Font Selection for internal CGROM
    When FNCR0 B7 = 0 and B5 = 0, Internal CGROM supports the
...
void ER_TFTBasic::Font_Background_select_Original_Canvas(void)
{
    /*
    Font Transparency
...
//[CEh]=========================================================================
void ER_TFTBasic::GTFont_Select_GT21L16T1W(void)
{
    /*
...
void ER_TFTBasic::GTFont_Select_GT30L16U2W(void)
{
    /*
    GT Serial Font ROM Select
    000b: GT21L16T1W
...
void ER_TFTBasic::GTFont_Select_GT30L24T3Y(void)
{
    /*
    GT Serial Font ROM Select
    000b: GT21L16T1W
...
void ER_TFTBasic::GTFont_Select_GT30L24M1Z(void)
{
    /*
    GT Serial Font ROM Select
    000b: GT21L16T1W
...
void ER_TFTBasic::GTFont_Select_GT30L32S4W(void)
{
    /*
    GT Serial Font ROM Select
    000b: GT21L16T1W
...
void ER_TFTBasic::GTFont_Select_GT20L24F6Y(void)
{
    /*
    GT Serial Font ROM Select
    000b: GT21L16T1W
...
void ER_TFTBasic::GTFont_Select_GT21L24S1W(void)
{
    /*
    GT Serial Font ROM Select
    000b: GT21L16T1W
...
//[CFh]=========================================================================
void ER_TFTBasic::Set_GTFont_Decoder(unsigned char temp)
{
    /*
    [bit7-3]
...
//[D2h]~[D4h]=========================================================================
void ER_TFTBasic::Foreground_RGB(unsigned char RED, unsigned char GREEN, unsigned char BLUE)
{
    /*
    ...
    */
    ...
    ER_TFT.LCD_DataWrite(RED);

    ER_TFT.LCD_CmdWrite(0xD3);
    ER_TFT.LCD_DataWrite(GREEN);

    ER_TFT.LCD_CmdWrite(0xD4);
    ER_TFT.LCD_DataWrite(BLUE);
}

// Input data format:R3G3B2
void ER_TFTBasic::Foreground_color_256(unsigned char temp)
{
    ER_TFT.LCD_CmdWrite(0xD2);
    ER_TFT.LCD_DataWrite(temp);
...
// Input data format:R8G8B8
void ER_TFTBasic::Foreground_color_16M(unsigned long temp)
{
...
*/
void ER_TFTBasic::Background_RGB(unsigned char RED, unsigned char GREEN, unsigned char BLUE)
{
...
// Input data format:R3G3B2
void ER_TFTBasic::Background_color_256(unsigned char temp)
{
    ER_TFT.LCD_CmdWrite(0xD5);
    ER_TFT.LCD_DataWrite(temp);
...
// Input data format:R8G8B8
void ER_TFTBasic::Background_color_16M(unsigned long temp)
{
    ER_TFT.LCD_CmdWrite(0xD5);
    ER_TFT.LCD_DataWrite(temp >> 16);
...
11: Sleep Mode
*/
void ER_TFTBasic::Power_Normal_Mode(void)
{
    ER_TFT.LCD_CmdWrite(0xDF);
    ER_TFT.LCD_DataWrite(0x00);
    Check_IC_ready();
}
void ER_TFTBasic::Power_Saving_Standby_Mode(void)
{
    ER_TFT.LCD_CmdWrite(0xDF);
    //	ER_TFT.LCD_DataWrite(0x01);
    //	ER_TFT.LCD_CmdWrite(0xDF);
    ER_TFT.LCD_DataWrite(0x81);
}
void ER_TFTBasic::Power_Saving_Suspend_Mode(void)
{
    ER_TFT.LCD_CmdWrite(0xDF);
    //	ER_TFT.LCD_DataWrite(0x02);
    //	ER_TFT.LCD_CmdWrite(0xDF);
    ER_TFT.LCD_DataWrite(0x82);
}
void ER_TFTBasic::Power_Saving_Sleep_Mode(void)
{
    ER_TFT.LCD_CmdWrite(0xDF);
    ER_TFT.LCD_DataWrite(0x03);
    ER_TFT.LCD_CmdWrite(0xDF);
    ER_TFT.LCD_DataWrite(0x83);
}

//[E5h]~[E6h]=========================================================================
void ER_TFTBasic::RA8889_I2CM_Clock_Prescale(unsigned short WX)
{
    /*
    ...
    */
    ER_TFT.LCD_CmdWrite(0xE5);
    ER_TFT.LCD_DataWrite(WX);
    ER_TFT.LCD_CmdWrite(0xE6);
    ER_TFT.LCD_DataWrite(WX >> 8);
}
//[E7h]=========================================================================
void ER_TFTBasic::RA8889_I2CM_Transmit_Data(unsigned char temp)
{
    /*
    I2C Master Transmit[7:0]
    */
    ER_TFT.LCD_CmdWrite(0xE7);
    ER_TFT.LCD_DataWrite(temp);
}
//[E8h]=========================================================================
...
//[bit7] START
void ER_TFTBasic::RA8889_I2CM_Write_With_Start(void)
{
    ER_TFT.LCD_CmdWrite(0xE9);
#ifdef Disable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x90);
#endif

#ifdef Enable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x91);
#endif
}

//[bit6] STOP
void ER_TFTBasic::RA8889_I2CM_Stop(void)
{
    ER_TFT.LCD_CmdWrite(0xE9);
#ifdef Disable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x40);
#endif

#ifdef Enable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x41);
#endif
}

//[bit5] READ
void ER_TFTBasic::RA8889_I2CM_Read_With_Ack(void)
{
    ER_TFT.LCD_CmdWrite(0xE9);
#ifdef Disable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x20);
#endif
#ifdef Enable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x21);
#endif
}

void ER_TFTBasic::RA8889_I2CM_Read_With_Nack(void)
{
    ER_TFT.LCD_CmdWrite(0xE9);
#ifdef Disable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x68);
#endif
#ifdef Enable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x69);
#endif
}

//[bit4] WRITE
void ER_TFTBasic::RA8889_I2CM_Write(void)
{
    ER_TFT.LCD_CmdWrite(0xE9);
#ifdef Disable_I2CM_Noise_Filter
    ER_TFT.LCD_DataWrite(0x10);
...
//[F0h]=========================================================================
void ER_TFTBasic::Set_GPIO_A_In_Out(unsigned char temp)
{
    /*
...
//[F3h]=========================================================================
void ER_TFTBasic::Set_GPIO_C_In_Out(unsigned char temp)
{
    /*
    ...
    1: Input
    */
    ER_TFT.LCD_CmdWrite(0xF3);
    ER_TFT.LCD_DataWrite(temp);
}
//[F4h]=========================================================================
void ER_TFTBasic::Write_GPIO_C_7_0(unsigned char temp)
{
    /*
    REG[F4h] GPIO-C
    GPIO-C[7] : XPWM0,
...
//[F5h]=========================================================================
void ER_TFTBasic::Set_GPIO_D_In_Out(unsigned char temp)
{
    /*
    ...
    1: Input
    */
    ER_TFT.LCD_CmdWrite(0xF5);
    ER_TFT.LCD_DataWrite(temp);
}
//[F6h]=========================================================================
void ER_TFTBasic::Write_GPIO_D_7_0(unsigned char temp)
{
    /*
    GPIO-D[7:0] : General Purpose Input/Output
    */
    ER_TFT.LCD_CmdWrite(0xF6);
    ER_TFT.LCD_DataWrite(temp);
}
...
//[F7h]=========================================================================
void ER_TFTBasic::Set_GPIO_E_In_Out(unsigned char temp)
{
    /*
    ...
    1: Input
    */
    ER_TFT.LCD_CmdWrite(0xF7);
    ER_TFT.LCD_DataWrite(temp);
}
//[F8h]=========================================================================
void ER_TFTBasic::Write_GPIO_E_7_0(unsigned char temp)
{
    /*
    GPIO-E[7:0] : General Purpose Input/Output.
    share with {PDAT[23:19], PDAT[15:13]}
    */
    ER_TFT.LCD_CmdWrite(0xF8);
    ER_TFT.LCD_DataWrite(temp);
}
...
//[F9h]=========================================================================
void ER_TFTBasic::Set_GPIO_F_In_Out(unsigned char temp)
{
    /*
    ...
    1: Input
    */
    ER_TFT.LCD_CmdWrite(0xF9);
    ER_TFT.LCD_DataWrite(temp);
}
//[FAh]=========================================================================
void ER_TFTBasic::Write_GPIO_F_7_0(unsigned char temp)
{
    /*
    GPIO-F[7:0] : General Purpose Input/Output.
    share with {XPDAT[12:10], XPDAT[7:3]}
    */
    ER_TFT.LCD_CmdWrite(0xFA);
    ER_TFT.LCD_DataWrite(temp);
}
...
void ER_TFTBasic::Long_Key_enable(void)
{
    /*
   ...
   */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0xFB);
    temp = ER_TFT.LCD_DataRead();
    temp |= cSetb6;
    ER_TFT.LCD_DataWrite(temp);
}

void ER_TFTBasic::Key_Scan_Freg(unsigned char setx)
{
...
void ER_TFTBasic::Key_Scan_Wakeup_Function_Enable(void)
{
    /*
    ...
    */
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0xFC);
    temp = ER_TFT.LCD_DataRead();
    temp |= cSetb7;
    ER_TFT.LCD_DataWrite(temp);
}

void ER_TFTBasic::Long_Key_Timing_Adjustment(unsigned char setx)
{
    /*Long Key Timing Adjustment*/
    unsigned char temp, temp1;
    temp = setx & 0x1c;
    ER_TFT.LCD_CmdWrite(0xFC);
    temp1 = ER_TFT.LCD_DataRead();
    temp1 |= temp;
    ER_TFT.LCD_DataWrite(temp1);
}
...
unsigned char Numbers_of_Key_Hit(void)
{
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0xFC);
    temp = ER_TFT.LCD_DataRead(); // read key touch number
...
//[46h]=========================================================================
void ER_TFTBasic::SPIM_Clock_Divided_2(void) //(default)
{
    /*
    ...
    */
    ...
    ER_TFT.REG_Switch_to_REG1();

    ER_TFT.LCD_CmdWrite(0x46);
    temp = ER_TFT.LCD_DataRead();
    temp &= cClrb2;
    ER_TFT.LCD_DataWrite(temp);

    ER_TFT.REG_Switch_to_REG0();
}
void ER_TFTBasic::SPIM_Clock_Divided_1(void) //
{
    /*
    REG[46h] [bit2] Spi master frequency eliminator
...
void ER_TFTBasic::SPIM_Selectable_Bus_mode(void) //(default)
{
    /*
...
void ER_TFTBasic::SPIM_Fixed_Bus_mode(void) //
{
    /*
    REG[46h] [bit1] SPI_Master_Separate
    0: bus ???(可選擇)
...
void ER_TFTBasic::REG_Switch_to_REG0(void) //(default)
{
    /*
...
void ER_TFTBasic::REG_Switch_to_REG1(void) //
{
    /*
    REG[46h] [bit0] Page switch, and SPIM bus switch.
    0: page 0, SPIM bus 0.
...
void ER_TFTBasic::Select_SFI_2(void)
{
    /*[bit7]
    Serial Flash/ROM I/F # Select
    0: Serial Flash/ROM 0 I/F is selected.
...
void ER_TFTBasic::Select_SFI_3(void)
{
    /*[bit7]
    Serial Flash/ROM I/F # Select
    0: Serial Flash/ROM 0 I/F is selected.
...
void ER_TFTBasic::Select_nSS_drive_on_xnsfcs2(void)
{
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0xB9);
    temp = ER_TFT.LCD_DataRead();
...
void ER_TFTBasic::Select_nSS_drive_on_xnsfcs3(void)
{
    unsigned char temp;
    ER_TFT.LCD_CmdWrite(0xB9);
    temp = ER_TFT.LCD_DataRead();
...
//[REG PAGE 1]
//[0Bh]=========================================================================
void ER_TFTBasic::Disable_IDEC_Interrupt(void)
{
    /* [REG PAGE 1], [0Bh]
...
void ER_TFTBasic::Enable_IDEC_Interrupt(void)
{
    /* [REG PAGE 1], [0Bh]
    [bit0] IDEC Interrupt Enable Bit
    0: Disable Interrupt.
...
//[REG PAGE 1]
//[0Ch]=========================================================================
void ER_TFTBasic::Clear_IDEC_Interrupt(void)
{
    /* [REG PAGE 1], [0Ch]
...
//[REG PAGE 1]
//[0Dh]=========================================================================
void ER_TFTBasic::Unmask_IDEC_Interrupt(void)
{
    /* [REG PAGE 1], [0Dh]
...
void ER_TFTBasic::Mask_IDEC_Interrupt(void)
{
    /* [REG PAGE 1], [0Dh]
    [bit0] Mask IDEC Interrupt Flag
    0: Unmask.
...
void ER_TFTBasic::AVI_Shadow_PIP_Start_Address(unsigned long Addr)
{
    /*[REG PAGE 1]
...
void ER_TFTBasic::Check_Busy_Media_Decode(void)
{
    /*[REG PAGE 1], REG[A0h],
...
unsigned char ER_TFTBasic::Read_Media_Decode_Busy(void)
{
    /*[REG PAGE 1], REG[A0h],
    [bit6] Media decode busy
    */
...
unsigned char ER_TFTBasic::Check_Media_Fifo_Flag(void)
{
    /*[REG PAGE 1], REG[A0h],
...
//[REG PAGE 1]
//[A1h]~[A8h]=========================================================================
unsigned short ER_TFTBasic::Read_Media_Height(void)
{
    /*[REG PAGE 1],
    ...
    */
    ...
    ER_TFT.LCD_CmdWrite(0xA1);
    temp = ER_TFT.LCD_DataRead();

    ER_TFT.LCD_CmdWrite(0xA2);
    Media_height = ER_TFT.LCD_DataRead();
...
unsigned short ER_TFTBasic::Read_Media_Width(void)
{
    /*[REG PAGE 1],
    ...
    */
    ...
    ER_TFT.LCD_CmdWrite(0xA3);
    temp = ER_TFT.LCD_DataRead();

    ER_TFT.LCD_CmdWrite(0xA4);
    Media_Width = ER_TFT.LCD_DataRead();
...
unsigned long ER_TFTBasic::Read_Media_Data_Amount(void)
{
    /*[REG PAGE 1],
    ...
    */
    ...
    ER_TFT.LCD_CmdWrite(0xA5);
    temp4 = ER_TFT.LCD_DataRead();

    ER_TFT.LCD_CmdWrite(0xA6);
    temp3 = ER_TFT.LCD_DataRead();

    ER_TFT.LCD_CmdWrite(0xA7);
    temp2 = ER_TFT.LCD_DataRead();

    ER_TFT.LCD_CmdWrite(0xA8);
    temp1 = ER_TFT.LCD_DataRead();
...
//[REG PAGE 1]
//[A9h]=========================================================================
void ER_TFTBasic::Reset_IDEC(void)
{
...
//[REG PAGE 1]
//[B6h]=========================================================================
void ER_TFTBasic::IDEC_Select_SFI_0(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit7-6]
...
void ER_TFTBasic::IDEC_Select_SFI_1(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit7-6]
    00: Serial Flash/ROM 0 I/F is selected.
...
void ER_TFTBasic::IDEC_Select_SFI_2(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit7-6]
    00: Serial Flash/ROM 0 I/F is selected.
...
void ER_TFTBasic::IDEC_Select_SFI_3(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit7-6]
    00: Serial Flash/ROM 0 I/F is selected.
...
//[B6h][bit4-3]====
void ER_TFTBasic::FONT_DMA_Select_Bus_0(void)
{
    /*[REG PAGE 1], REG[B6h],
...
void ER_TFTBasic::FONT_DMA_Select_Bus_1(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit4] FONT/DMA serial flash sck and data bus select
    0: SPI bus 0 is selected and the relative pins (xmosi, xmiso, xsio2, xsio3) are active.
...
void ER_TFTBasic::IDEC_Select_Bus_0(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit3]
...
void ER_TFTBasic::IDEC_Select_Bus_1(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit3]
    IDEC sck and data bus select
...
//[B6h][bit2-1]====
void ER_TFTBasic::IDEC_Destination_Color_8bpp(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit2-1]
...
void ER_TFTBasic::IDEC_Destination_Color_16bpp(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit2-1]
    IDEC destination Color depth:
...
void ER_TFTBasic::IDEC_Destination_Color_24bpp(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit2-1]
    IDEC destination Color depth:
...
//[B6h][bit0]====
void ER_TFTBasic::IDEC_Starts_Decoding(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit0]
...
void ER_TFTBasic::Check_Busy_IDEC(void)
{
    /*[REG PAGE 1], REG[B6h],
    [bit0]
    Write Function: IDEC Start Bit
...
void ER_TFTBasic::IDEC_Select_SFI_24bit_Address(void)
{
    /*[REG PAGE 1],[B7h] [bit5]
...
void ER_TFTBasic::IDEC_Select_SFI_32bit_Address(void)
{
    /*[REG PAGE 1],[B7h] [bit5]
    IDEC Serial Flash/ROM Address Mode
    0: 24 bits address mode
...
void ER_TFTBasic::IDEC_Select_standard_SPI_Mode0_or_Mode3(void)
{
    /*[REG PAGE 1],[B7h] [bit4]
...
void ER_TFTBasic::IDEC_Select_RA8875_SPI_Mode0_and_Mode3(void)
{
    /*[REG PAGE 1],[B7h] [bit4]
    IDEC Serial Interface
    0: standard SPI mode 0 or mode 3 timing
...
void ER_TFTBasic::IDEC_Select_SFI_Single_Mode_Dummy_0T_03h(void) //
{
    /*[REG PAGE 1],[B7h]
    [bit3-0]
...
void ER_TFTBasic::IDEC_Select_SFI_Single_Mode_Dummy_8T_0Bh(void) //
{
    /*[REG PAGE 1],[B7h]
    [bit3-0]
    0000b: 1x read command code = 03h. Without dummy cycle between address and data.
...
void ER_TFTBasic::IDEC_Select_SFI_Single_Mode_Dummy_16T_1Bh(void) //
{
    /*[REG PAGE 1],[B7h]
    [bit3-0]
    0000b: 1x read command code = 03h. Without dummy cycle between address and data.
...
void ER_TFTBasic::IDEC_Select_SFI_Dual_Mode_Dummy_8T_3Bh(void) //
{
    /*[REG PAGE 1],[B7h]
    [bit3-0]
...
void ER_TFTBasic::IDEC_Select_SFI_Dual_Mode_Dummy_4T_BBh(void) //
{
    /*[REG PAGE 1],[B7h]
    [bit3-0]
    0110b: 2x read command code = 3Bh. 8 dummy cycles inserted between address and data phase. (mode 0)
...
void ER_TFTBasic::IDEC_Select_SFI_Quad_Mode_Dummy_8T_6Bh(void) //
{
    /*[REG PAGE 1],[B7h]
    [bit3-0]
...
void ER_TFTBasic::IDEC_Select_SFI_Quad_Mode_Dummy_4T_EBh(void) //
{
    /*[REG PAGE 1],[B7h]
    [bit3-0]
    1010b: 4x read command code – 6Bh.
...
//[REG PAGE 1]
//[BBh]=========================================================================
void ER_TFTBasic::IDEC_SPI_Clock_Divide(unsigned char IDEC_SPIclock_divide) //
{
    ...
    */
    ...
    ER_TFT.LCD_CmdWrite(0xBB);
    ER_TFT.LCD_DataWrite(IDEC_SPIclock_divide);
...
//[REG PAGE 1]
//[BCh]~[BFh]=========================================================================
void ER_TFTBasic::IDEC_Source_Start_Address(unsigned long Addr) // AVI,JPG,BMP	//
{
    /*[REG PAGE 1]
...
//[REG PAGE 1]
//[C0h][C1h][C2h][C3h]=========================================================================
void ER_TFTBasic::IDEC_Destination_Upper_Left_Corner(unsigned short WX, unsigned short HY)
{
    /*
...
//[REG PAGE 1]
//[C5h]=========================================================================
void ER_TFTBasic::IDEC_AVI_Decoding_to_PIP1_and_Shadow(void)
{
    /*[REG PAGE 1],[C5h]
    [bit1-0]
    ...
    */
    ...
    ER_TFT.LCD_CmdWrite(0xC5);
    ER_TFT.LCD_DataWrite(0x00);

    ER_TFT.REG_Switch_to_REG0();
}
void ER_TFTBasic::IDEC_AVI_Decoding_to_PIP2_and_Shadow(void)
{
    /*[REG PAGE 1],[C5h]
    [bit1-0]
    00b: AVI_Decoding_to_PIP1_and_Shadow
...
void ER_TFTBasic::IDEC_AVI_Decoding_to_PIP1_no_Shadow(void)
{
    /*[REG PAGE 1],[C5h]
    [bit1-0]
    00b: AVI_Decoding_to_PIP1_and_Shadow
...
//[REG PAGE 1]
//[C6h][C7h][C8h][C9h]=========================================================================
void ER_TFTBasic::IDEC_Transfer_Number(unsigned long Num)
{
    /*
...
//[REG PAGE 1]
//[CAh][CBh][CCh][CDh]=========================================================================
void ER_TFTBasic::IDEC_Destination_Start_Address(unsigned long Addr) // ,JPG,BMP	//
{
    /*[REG PAGE 1]
...
//[REG PAGE 1]
//[CEh][CFh]=========================================================================
void ER_TFTBasic::IDEC_Destination_Image_Width(unsigned short WX) // ,JPG,BMP	//
{
    /*[REG PAGE 1]
...
//[REG PAGE 0]
//[C5h]=========================================================================
void ER_TFTBasic::SPIM_Select_Bus_0(void)
{
    /*[REG PAGE 0],[C5h]
...
void ER_TFTBasic::SPIM_Select_Bus_1(void) //
{
    /*[REG PAGE 0],[C5h]
    [bit7] SPI master bus select
    0b:  Bus 0
...
void ER_TFTBasic::SPI_Master_LatchEdge_Select_Rising(void)
{
    /*[REG PAGE 0],[C5h]
...
void ER_TFTBasic::SPI_Master_LatchEdge_Select_Falling(void)
{
    /*[REG PAGE 0],[C5h]
    [bit5] SPI master rx register latch edge
    0: cclk rising edge
...
//[REG PAGE 1]
//[D3h]=========================================================================
void ER_TFTBasic::Press_AVI_Pause(void)
{
    /*[REG PAGE 1], REG[D3h],
...
//[REG PAGE 1]
//[D4h]=========================================================================
void ER_TFTBasic::Stop_AVI(void)
{
    /*[REG PAGE 1], REG[D4h],
...
void ER_TFTBasic::Select_Font_Height_WxN_HxN_ChromaKey_Alignment(
...
{
    if (Font_Height == 16) ER_TFT.Font_Select_8x16_16x16();
    if (Font_Height == 24) ER_TFT.Font_Select_12x24_24x24();
    if (Font_Height == 32) ER_TFT.Font_Select_16x32_32x32();

    //(*)
    if (XxN == 1) ER_TFT.Font_Width_X1();
    if (XxN == 2) ER_TFT.Font_Width_X2();
    if (XxN == 3) ER_TFT.Font_Width_X3();
    if (XxN == 4) ER_TFT.Font_Width_X4();

    //(*)
    if (YxN == 1) ER_TFT.Font_Height_X1();
    if (YxN == 2) ER_TFT.Font_Height_X2();
    if (YxN == 3) ER_TFT.Font_Height_X3();
    if (YxN == 4) ER_TFT.Font_Height_X4();

    //(*)
    if (ChromaKey == 0) { ER_TFT.Font_Background_select_Color(); }

    if (ChromaKey == 1) { ER_TFT.Font_Background_select_Original_Canvas(); }

    //(*)
    if (Alignment == 0) { ER_TFT.Disable_Font_Alignment(); }

    if (Alignment == 1) { ER_TFT.Enable_Font_Alignment(); }
}
...
void ER_TFTBasic::SPI_NOR_Flash_switch_QUAD_MODE(
...
)
{
    ...
    ER_TFT.SPI_Master_LatchEdge_Select_Falling();
    ER_TFT.SPIM_Clock_Divided_1();
    ER_TFT.SPI_Clock_Period(2);

    switch (BUS) {
        case 0:
            ER_TFT.SPIM_Select_Bus_0();
            break;
        case 1:
            ER_TFT.SPIM_Select_Bus_1();
            break;
    }

    switch (SCS) {
        case 0:
            ER_TFT.Select_nSS_drive_on_xnsfcs0();
            break;

        case 1:
            ER_TFT.Select_nSS_drive_on_xnsfcs1();
            break;

        case 2:
            ER_TFT.Select_nSS_drive_on_xnsfcs2();
            break;

        case 3:
            ER_TFT.Select_nSS_drive_on_xnsfcs3();
            break;
    }
    ...
    // WREN : write enable
    ER_TFT.nSS_Active();
    ER_TFT.SPI_Master_FIFO_Data_Put(0x06);
    ER_TFT.nSS_Inactive();
    ...
    do {
        ER_TFT.nSS_Active();
        ER_TFT.SPI_Master_FIFO_Data_Put(0x05); //
        temp = ER_TFT.SPI_Master_FIFO_Data_Put(0xff);
        ER_TFT.nSS_Inactive();
        delay(1);
    ...
    switch (flash) {
        case 0:
            ...
            // WRITE STATUS REG
            ER_TFT.nSS_Active();
            ER_TFT.SPI_Master_FIFO_Data_Put(0x01);
            ER_TFT.SPI_Master_FIFO_Data_Put(0x40);
            ER_TFT.nSS_Inactive();
            ///*************************************
            break;
        ...
        case 1:
            ...
            // WRITE STATUS REG
            ER_TFT.nSS_Active();
            ER_TFT.SPI_Master_FIFO_Data_Put(0x31); // Status Register-2
            ER_TFT.SPI_Master_FIFO_Data_Put(0x02); // QE=1, Quad Mode
            ER_TFT.nSS_Inactive();
            ///*****************************
            break;
        case 2:
            ...
            ER_TFT.SPI_Master_FIFO_Data_Put(0x01);
            ER_TFT.SPI_Master_FIFO_Data_Put(0x31); // Status Register-2
            ER_TFT.SPI_Master_FIFO_Data_Put(0x38);
            ER_TFT.nSS_Inactive();
            ///*****************************
            break;
...
void ER_TFTBasic::SPI_NOR_initial_DMA(
...
)
{
    ...
    switch (BUS) {
        case 0:
            ER_TFT.FONT_DMA_Select_Bus_0();
            ER_TFT.SPIM_Select_Bus_0();
            break;

        case 1:
            ER_TFT.FONT_DMA_Select_Bus_1();
            ER_TFT.SPIM_Select_Bus_1();
            break;
    }
    ...
    ER_TFT.Select_RA8875_SPI_Mode0_and_Mode3();

    switch (mode) {
        case 0:
            ER_TFT.Select_SFI_Single_Mode_Dummy_0T_03h(); // single mode
            break;

        case 1:
            ER_TFT.Select_SFI_Single_Mode_Dummy_8T_0Bh(); // single mode
            break;

        case 2:
            ER_TFT.Select_SFI_Single_Mode_Dummy_16T_1Bh(); // single mode
            break;

        case 3:
            ER_TFT.Select_SFI_Dual_Mode_Dummy_8T_3Bh(); // dual mode
            break;

        case 4:
            ER_TFT.SPI_NOR_Flash_switch_QUAD_MODE(BUS, SCS, flash); // quad mode
            ER_TFT.Select_SFI_Quad_Mode_Dummy_8T_6Bh();
            break;

        case 5:
            ER_TFT.SPI_NOR_Flash_switch_QUAD_MODE(BUS, SCS, flash); // quad mode
            ER_TFT.Select_SFI_Quad_Mode_Dummy_4T_EBh();
    }

    ER_TFT.Select_SFI_DMA_Mode();

    if (addr_24b_32b == 0)
        ER_TFT.Select_SFI_24bit_Address();
    else {
        ER_TFT.switch_24bits_to_32bits(BUS, SCS);
        ER_TFT.Select_SFI_32bit_Address();
    }
...
{
    ER_TFT.SPIM_Clock_Divided_1();                // SPI Clock = System Clock /{(Clk)*2}
    ER_TFT.SPI_Master_LatchEdge_Select_Falling(); // SPIM latch

    ER_TFT.Select_SFI_DMA_Mode(); // Set Serial Flash DMA Mode
    ER_TFT.SPI_Clock_Period(Clk);
    /////////////////////////////////////////****************DMA
    ER_TFT.Goto_Pixel_XY(X1, Y1);                         // set Memory coordinate in Graphic Mode
    ER_TFT.SFI_DMA_Destination_Upper_Left_Corner(X1, Y1); // DMA Destination position(x,y)
    ER_TFT.SFI_DMA_Transfer_Width_Height(X_W, Y_H);       // Set DMA Block (Height , Width)
    ER_TFT.SFI_DMA_Source_Width(P_W);                     // Set DMA Source Picture Width
    ER_TFT.SFI_DMA_Source_Start_Address(Addr);            // Set Serial Flash DMA Source Starting Address

    ER_TFT.Start_SFI_DMA();      // Start DMA
    ER_TFT.Check_Busy_SFI_DMA(); // DMA Busy Check
}

void ER_TFTBasic::DMA_32bit(
...
)
{
    ...
    ER_TFT.Select_SFI_32bit_Address(); // Set Serial Flash/ROM 32bits Address
    /////////////////////////////////////////****************DMA
    ER_TFT.Goto_Pixel_XY(X1, Y1);                         // set Memory coordinate in Graphic Mode
    ER_TFT.SFI_DMA_Destination_Upper_Left_Corner(X1, Y1); // DMA Destination position(x,y)
    ER_TFT.SFI_DMA_Transfer_Width_Height(X_W, Y_H);       // Set DMA Block (Height , Width)
...
void ER_TFTBasic::switch_24bits_to_32bits(unsigned char BUS, unsigned char SCS)
{
    ER_TFT.Enable_SFlash_SPI();
    ER_TFT.SPI_Master_LatchEdge_Select_Falling();
    ER_TFT.SPIM_Clock_Divided_1();
    ...
    ER_TFT.nSS_Active(); // nSS port will goes low
    ER_TFT.SPI_Master_FIFO_Data_Put(0xB7);

    delay(1);
    ER_TFT.nSS_Inactive(); // nSS port will goes high
}


void ER_TFTBasic::SPI_NOR_initial_JPG_AVI(
    unsigned char flash // 0 : MXIC , 1 : Winbond
    ,
...
)
{
        ...
        ER_TFT.SPI_Master_LatchEdge_Select_Falling();
        ER_TFT.SPIM_Clock_Divided_1();
        ER_TFT.IDEC_SPI_Clock_Divide(SCK_Divide);
        ER_TFT.SPI_Clock_Period(2);

    switch (BUS) {
        case 0:
                ER_TFT.IDEC_Select_Bus_0(); // MEDIA ?I?IBUS 0
                ER_TFT.SPIM_Select_Bus_0();
            break;
        case 1:
                ER_TFT.IDEC_Select_Bus_1(); // MEDIA ?I?IBUS 1
                ER_TFT.SPIM_Select_Bus_1();
            break;
    }
    ...
    switch (SCS) {
        case 0:
                ER_TFT.IDEC_Select_SFI_0(); // MEDIA?I?ICS0
                ER_TFT.Select_nSS_drive_on_xnsfcs0();
            break;

        case 1:
                ER_TFT.IDEC_Select_SFI_1(); // MEDIA?I?ICS1
                ER_TFT.Select_nSS_drive_on_xnsfcs1();
            break;

        case 2:
                ER_TFT.IDEC_Select_SFI_2(); // MEDIA?I?ICS2
                ER_TFT.Select_nSS_drive_on_xnsfcs2();
            break;

        case 3:
                ER_TFT.IDEC_Select_SFI_3(); // MEDIA?I?ICS3
                ER_TFT.Select_nSS_drive_on_xnsfcs3();
            break;
    }

        ER_TFT.SPI_NOR_Flash_switch_QUAD_MODE(BUS, SCS, flash); // Into Quad Mode

    if (addr_24b_32b == 0)
            ER_TFT.IDEC_Select_SFI_24bit_Address();
    else {
            ER_TFT.switch_24bits_to_32bits(BUS, SCS);
            ER_TFT.IDEC_Select_SFI_32bit_Address();
    }

        ER_TFT.IDEC_Select_RA8875_SPI_Mode0_and_Mode3();
        ER_TFT.IDEC_Select_SFI_Quad_Mode_Dummy_8T_6Bh(); // MEDIA QUAD MODE
    // IDEC_Select_SFI_Quad_Mode_Dummy_4T_EBh();

        ER_TFT.IDEC_AVI_Decoding_to_PIP1_and_Shadow();
}        
        
void ER_TFTBasic::JPG_NOR(
...
)
{
    ...
    ER_TFT.IDEC_Destination_Image_Width(IDEC_canvas_width);
    ER_TFT.IDEC_Destination_Upper_Left_Corner(x, y);

    ER_TFT.IDEC_Source_Start_Address(addr); // JPG癬cl|i
    ER_TFT.IDEC_Transfer_Number(JPGsize);   // JPG SIZE
    ER_TFT.IDEC_Starts_Decoding();          //?}cl?NJPG
    ER_TFT.Check_Busy_Media_Decode();
}

void ER_TFTBasic::AVI_NOR(
...
)
{
    ...
    ER_TFT.IDEC_Source_Start_Address(addr);
    ER_TFT.IDEC_Transfer_Number(videosize); // AVI File Size

    ER_TFT.AVI_Shadow_PIP_Start_Address(shadow_buffer_addr);

    ER_TFT.Select_PIP1_Parameter();

    ER_TFT.PIP_Image_Start_Address(PIP_buffer_addr);
    ER_TFT.PIP_Display_Start_XY(x, y);
    ER_TFT.PIP_Image_Width(PIP_width);
    ER_TFT.PIP_Window_Image_Start_XY(0, 0);
    ER_TFT.PIP_Window_Width_Height(width, height);
    //	Enable_PIP1();
    // Check_Vsync_finished();
    ER_TFT.IDEC_Starts_Decoding();
    // Check_Busy_Media_Decode();
}        
        

void ER_TFTBasic::BTE_Alpha_Blending_Picture_Mode(unsigned long S0_Addr, unsigned short S0_W, unsigned short XS0, unsigned short YS0, unsigned long S1_Addr, unsigned short S1_W, unsigned short XS1, unsigned short YS1, unsigned long Des_Addr, unsigned short Des_W, unsigned short XDes, unsigned short YDes, unsigned short X_W, unsigned short Y_H, unsigned char alpha)
{
    ER_TFT.BTE_S0_Memory_Start_Address(S0_Addr);
    ER_TFT.BTE_S0_Image_Width(S0_W);
    ER_TFT.BTE_S0_Window_Start_XY(XS0, YS0);

    ER_TFT.BTE_S1_Memory_Start_Address(S1_Addr);
    ER_TFT.BTE_S1_Image_Width(S1_W);
    ER_TFT.BTE_S1_Window_Start_XY(XS1, YS1);

    ER_TFT.BTE_Destination_Memory_Start_Address(Des_Addr);
    ER_TFT.BTE_Destination_Image_Width(Des_W);
    ER_TFT.BTE_Destination_Window_Start_XY(XDes, YDes);

    ER_TFT.BTE_Window_Size(X_W, Y_H);
    ER_TFT.BTE_Operation_Code(0x0A); // BTE Operation: Memory write with opacity (w/o ROP)
    ER_TFT.BTE_Alpha_Blending_Effect(alpha);
    ER_TFT.BTE_Enable();
    ER_TFT.Check_BTE_Busy();
}

void ER_TFTBasic::AVI_window(unsigned char ON_OFF )// 0 : turn off AVI window, 1 :turn on AVI window
{
    if (ON_OFF == 0) {
        ER_TFT.Disable_PIP1();
    }
...
void ER_TFTBasic::BTE_Memory_Copy(
...
)
{
    ...
    ER_TFT.BTE_ROP_Code(ROP_Code);
    ER_TFT.BTE_Operation_Code(0x02); // BTE Operation: Memory copy (move) with ROP.
    ER_TFT.BTE_Window_Size(X_W, Y_H);
    ER_TFT.BTE_Enable();
    ER_TFT.Check_BTE_Busy();
}
...
void ER_TFTBasic::SPI_NOR_DMA_png(
    ...
    unsigned long picture_Height)
{
    ...
    ER_TFT.SFI_DMA_Source_Start_Address(dma_page_addr);
    ER_TFT.SFI_DMA_Transfer_Number(picture_Width * picture_Height * 4); // picture Width x Height x Color depth(32bit = 4byte)
    ER_TFT.SFI_DMA_Destination_Start_Address(pic_buffer_Layer);         //
 ...
 void ER_TFTBasic::BTE_alpha_blending_32bit_Pixel_mode(
...
)
{
    ...
    ER_TFT.BTE_S0_Memory_Start_Address(S0_Start_Addr);
    ER_TFT.BTE_S0_Image_Width(S0_canvas_width);

    ER_TFT.BTE_S1_Color_32bit_Alpha();
    ER_TFT.BTE_S1_Memory_Start_Address(pic_buffer_Layer);
    ER_TFT.BTE_S1_Image_Width(picture_Width); // ARGB

    ER_TFT.BTE_Destination_Color_16bpp();
    ER_TFT.BTE_Destination_Memory_Start_Address(DES_Start_Addr);
    ER_TFT.BTE_Destination_Image_Width(DES_canvas_width);

    ER_TFT.BTE_S1_Window_Start_XY(0, 0);
    ER_TFT.BTE_S0_Window_Start_XY(S0X, S0Y);
    ER_TFT.BTE_Destination_Window_Start_XY(desX, desY);
    ER_TFT.BTE_Window_Size(BTE_X, BTE_Y);
    ER_TFT.BTE_ROP_Code(12);
    ER_TFT.BTE_Operation_Code(0x0A);
    ER_TFT.BTE_Enable();
    ER_TFT.Check_BTE_Busy();

...
void System_Check_Temp(void)
{
	...
	do
	{
		...
		{
			//delay_ms(1); 					//if MCU too fast//­YMCU ³t«×¤Ó§Ö¡A¥²­n®É¨Ï¥Î
			LCD_CmdWrite(0x01);
			//delay_ms(1); 					//if MCU too fast//­YMCU ³t«×¤Ó§Ö¡A¥²­n®É¨Ï¥Î
			temp =LCD_DataRead();
			if((temp & 0x80)==0x80)
		...
		if(system_ok==0 && i==5)
		{
			//if not ready, hardware reset again.
			GPIO_SetBits(GPIOE, GPIO_Pin_1 );
			delay_ms(1);
			
			GPIO_ResetBits(GPIOE, GPIO_Pin_1);
			delay_us(30); // XnRST have to keep low at least 256 OSC clocks. 
...
*/ 
	unsigned short i;
	// Please according to your usage to modify delay(check) time. // ½Ð¨Ì¾Ú±zªº¨Ï¥Î±¡ªp­×§ï°õ¦æ®É¶¡
//	for(i=0;i<1000;i++)
//	{
...
/* 1: PLL enable; cannot change PLL parameter.*/
	...
	/*check PLL was ready ( Please according to your usage to modify. // ½Ð¨Ì¾Ú±zªº¨Ï¥Î±¡ªp­×§ï)	 */
	for(i=0;i<1000;i++)
	{
		LCD_CmdWrite(0x01);
		temp=LCD_DataRead();
...
Function bit will be configured for relative PIP window.
*/
	LCD_CmdWrite(0x2A);	LCD_DataWrite(WX);
	LCD_CmdWrite(0x2B);	LCD_DataWrite(WX>>8);
	LCD_CmdWrite(0x2C);	LCD_DataWrite(HY);
	LCD_CmdWrite(0x2D);	LCD_DataWrite(HY>>8);
}
//[2Eh][2Fh][30h][31h]=========================================================================
...
*/
	LCD_CmdWrite(0x2E);	LCD_DataWrite(Addr);
	LCD_CmdWrite(0x2F);	LCD_DataWrite(Addr>>8);
	LCD_CmdWrite(0x30);	LCD_DataWrite(Addr>>16);
	LCD_CmdWrite(0x31);	LCD_DataWrite(Addr>>24);
}
//[32h][33h]=========================================================================
...
Function bit will be configured for relative PIP window.
*/
	LCD_CmdWrite(0x32);	LCD_DataWrite(WX);
	LCD_CmdWrite(0x33);	LCD_DataWrite(WX>>8);
}
//[34h][35h][36h][37h]=========================================================================
...
Function bit will be configured for relative PIP window.
*/
	LCD_CmdWrite(0x34);	LCD_DataWrite(WX);
	LCD_CmdWrite(0x35);	LCD_DataWrite(WX>>8);
	LCD_CmdWrite(0x36);	LCD_DataWrite(HY);
	LCD_CmdWrite(0x37);	LCD_DataWrite(HY>>8);
}
//[38h][39h][3Ah][3Bh]=========================================================================
...
Function bit will be configured for relative PIP window.
*/
	LCD_CmdWrite(0x38);	LCD_DataWrite(WX);
	LCD_CmdWrite(0x39);	LCD_DataWrite(WX>>8);
	LCD_CmdWrite(0x3A);	LCD_DataWrite(HY);
	LCD_CmdWrite(0x3B);	LCD_DataWrite(HY>>8);
}
...
Reference main Window coordinate.
*/	
	LCD_CmdWrite(0x40);	LCD_DataWrite(WX);
	LCD_CmdWrite(0x41);	LCD_DataWrite(WX>>8);
	LCD_CmdWrite(0x42);	LCD_DataWrite(HY);
	LCD_CmdWrite(0x43);	LCD_DataWrite(HY>>8);
}
//[44h]=========================================================================
...
Reference Canvas image coordinate. Unit: Pixel
*/
	LCD_CmdWrite(0x5F);	LCD_DataWrite(Addr);
	LCD_CmdWrite(0x60);	LCD_DataWrite(Addr>>8);
	LCD_CmdWrite(0x61);	LCD_DataWrite(Addr>>16);
	LCD_CmdWrite(0x62);	LCD_DataWrite(Addr>>24);
}
...
/*[B6h]bit7-6=
00b: [B7h] B3-0
01b: 4x read command code ¡V 6Bh. 
10b: 4x read command code ¡V EBh. 
11b:NA
*/
	unsigned char temp;
	LCD_CmdWrite(0xB6);
...
void Set_LCD_Panel(void)
{
		...
		*/
			LCD_Horizontal_Non_Display(96);	//Blank=90~376, TYP.=320¡C
		...
		*/
			...
			LCD_HSYNC_Pulse_Width(100);// ¨è·N¥[¤jµ¹ 76+ ¥Î
		...
		*/
			...
			LCD_VSYNC_Start_Position(4);//
			LCD_VSYNC_Pulse_Width(50);// ¨è·N¥[¤jµ¹ 76+ ¥Î
	#endif
		...
		*/
		...
		/*	[16h][17h] : Figure 19-3 [HND]	Non Display or Back porch (pixels)		= (HNDR + 1) * 8 + HNDFTR  
			...
			[19h] :		 Figure 19-3 [HPW]	Pulse Width(pixels)						= (HPW + 1)x8
		*/
			LCD_Horizontal_Non_Display(38);//INNOLUX800x600¡A46¡C
			LCD_HSYNC_Start_Position(210);//INNOLUX800x600¡A16~354¡C
			LCD_HSYNC_Pulse_Width(8);//INNOLUX800x600¡A1~40C
		
		/*	[1Ch][1Dh] : Figure 19-3 [VND]	Non-Display Period(Line)	= (VNDR + 1)
			[1Eh] :		 Figure 19-3 [VST]	Start Position(Line)		= (VSTR + 1)
			[1Fh] :		 Figure 19-3 [VPW]	Pulse Width(Line)			= (VPWR + 1)
		*/
			LCD_Vertical_Non_Display(15);//INNOLUX800x600¡A23¡C
			LCD_VSYNC_Start_Position(12);//INNOLUX800x600¡A1~77¡C
			LCD_VSYNC_Pulse_Width(8);//INNOLUX800x600¡A1~20C
		...
		*/
		...
		LCD_HorizontalWidth_VerticalHeight(800,600);//ATM1040L3A-T1 800x600¡C

		/*	[16h][17h] : Figure 19-3 [HND]	Non Display or Back porch (pixels)		= (HNDR + 1) * 8 + HNDFTR  
			[18h] :		 Figure 19-3 [HST]	Start Position or Front porch (pixels)	= (HSTR + 1)x8
			[19h] :		 Figure 19-3 [HPW]	Pulse Width(pixels)						= (HPW + 1)x8
		*/
			LCD_Horizontal_Non_Display(38);	//ATM1040L3A-T1 800x600¡A46¡C
			LCD_HSYNC_Start_Position(210);	//ATM1040L3A-T1 800x600¡A16~354¡C
			LCD_HSYNC_Pulse_Width(8);		//ATM1040L3A-T1 800x600¡A1~40C
		
		/*	[1Ch][1Dh] : Figure 19-3 [VND]	Non-Display Period(Line)	= (VNDR + 1)
			[1Eh] :		 Figure 19-3 [VST]	Start Position(Line)		= (VSTR + 1)
			[1Fh] :		 Figure 19-3 [VPW]	Pulse Width(Line)			= (VPWR + 1)
		*/
			LCD_Vertical_Non_Display(15);	//ATM1040L3A-T1 800x600¡A23¡C
			LCD_VSYNC_Start_Position(12);	//ATM1040L3A-T1 800x600¡A1~77¡C
			LCD_VSYNC_Pulse_Width(8);		//ATM1040L3A-T1 800x600¡A1~20C	    
	...
	#ifdef BA104S01_100	//BOE_800x600(¨ÊªF¤è)// 18bpp LVDS
		...
		*/
		...
		LCD_HorizontalWidth_VerticalHeight(800,480);// 800x480¡C
...
//Auto_Refresh=(64*DRAM_FREQ*1000)/(8192);
	...
	Auto_Refresh=Auto_Refresh-2;//´£«e±Ò°Êrefresh¡AÁ×§K­è¦n¸I¨ì·¥­­½d³ò¡C// Start [refresh] in advance to avoid just reaching the limits.
...
/*[REG PAGE 1],[B7h] 
[bit3-0]
1010b: 4x read command code ¡V 6Bh. 
1100b: 4x read command code ¡V EBh. 
*/
	unsigned char temp;
...
*/
	...
	LCD_CmdWrite(0xCA);	LCD_DataWrite(Addr);
	LCD_CmdWrite(0xCB);	LCD_DataWrite(Addr>>8);
	LCD_CmdWrite(0xCC);	LCD_DataWrite(Addr>>16);
	LCD_CmdWrite(0xCD);	LCD_DataWrite(Addr>>24);
...
*/
	...
	LCD_CmdWrite(0xC5);
	temp = LCD_DataRead();
	temp &= 0xDF;			//³]©wREG c5h, bit 5=0, B1101_1111.
	LCD_DataWrite(temp);
}
...
*/
	...
	LCD_CmdWrite(0xC5);
	temp = LCD_DataRead();
	temp |= 0x20;			//³]©wREG c5h, bit 5=1, B0010_0000.
	LCD_DataWrite(temp);
}

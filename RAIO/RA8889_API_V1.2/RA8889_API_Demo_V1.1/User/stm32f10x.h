...
typedef struct
{
  __IO uint32_t IMR;	  //中斷屏蔽暫存器	   1為不被屏蔽 0相反  19位線
  __IO uint32_t EMR;	  //事件屏蔽暫存器	   1為不被屏蔽 0相反  19位線
  __IO uint32_t RTSR;     //上升沿触發選擇暫存器   1為允許 0相反 19位線
  __IO uint32_t FTSR;      //下升沿触發選擇暫存器  1為允許 0相反  19位線
  __IO uint32_t SWIER;	  //軟件中斷事件暫存器
  __IO uint32_t PR;		  //掛起暫存器
} EXTI_TypeDef;
...
typedef struct
{
  __IO uint32_t CRL;   //端口配置暫存器﹛低8位﹛
  __IO uint32_t CRH;   //端口配置暫存器﹛高8位﹛
  __IO uint32_t IDR;   //數据輸入暫存器
  __IO uint32_t ODR;   //數据輸出暫存器
  __IO uint32_t BSRR;  //端口置位复位暫存器
  __IO uint32_t BRR;   //端口复位暫存器
  __IO uint32_t LCKR;  //端口配置鎖定暫存器
} GPIO_TypeDef;
